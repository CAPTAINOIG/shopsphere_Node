...
const Identifier = () => {
  ...
  // Reusable function to restrict input to no spaces
  const restrictNoSpaces = async (e) => {
    const value = e.target.value;
    if (/\s/.test(value)) {
      e.target.value = value.replace(/\s/g, ''); // Remove all spaces
      await trigger('identifier'); // Trigger validation to show error
    }
  };
    ...
    <>
      <section className='h-screen'>
        <div className='bg-white py-3 shadow-lg'>
          <h1 className='lg:ps-16 ps-5 py-2 border-b border-gray-500 font-bold text-xl'>Identifier Page</h1>
          <form action="" className='lg:mx-20 mx-5' onSubmit={handleSubmit(onSubmit)}>
            <div>
              ...
              <small className='text-red-600 ms-[42%] lg:ms-[53%]'>{errors.identifier?.message}</small>
            </div>
            <div>
              <div className='flex justify-between my-2'>
              ...
              <small className='text-red-600 ms-[42%] lg:ms-[55%]'>{errors.description?.message}</small>
            </div>
