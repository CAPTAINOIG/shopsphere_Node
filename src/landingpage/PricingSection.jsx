...
const PricingSection = () => {
  ...
  useEffect(() => {
    const fetchPlans = async () => {
      try {
        const response = await axios.get('https://service.swiftsuite.app/accounts/account-tier/');
        const apiPlans = response.data.map(plan => {
          ...
          // Feature toggles now only display names
          const features = [
        ...
        <div className="grid lg:grid-cols-4 md:grid-cols-2 grid-cols lg:gap-5 md:gap-6 gap-4 lg:px-0 md:px-4 px-2">
          {plans.map((plan) => (
            ...
            >
              ...
              <p className={isLandingPage ? 'hidden' : 'my-2 flex items-start h-[5rem]'}>{plan.description}</p>
              ...
              <ul className="space-y-3 my-3">
                {plan.feature.map((feature) => (
                  <li key={feature.id} className="flex items-start">
                    <div className="flex-shrink-0 opacity-60" style={{ color: feature.value ? plan.color : '#999' }}>
