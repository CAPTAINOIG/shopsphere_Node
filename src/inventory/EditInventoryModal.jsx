...
const EditInventoryModal = ({ isOpen, onClose, inventoryEdit, setInventoryEdit, token, userId }) => {
  ...
  const handleEdit = async () => {
    const editDetails = {
      ebay_item_id: inventoryEdit.ebay_item_id || null,
  ...
  const handleInventoryDetails = async (editDetails) => {
    setLoader(true);
    try {
      const response = await axios.put(`https://service.swiftsuite.app/inventoryApp/update_ebay_item_details/520822/${userId}/`, editDetails,
        {
          headers: {
