...
const Faqs = () => {
  ...
  return (
    <div
      className={`mx-auto bg-[#E7F2ED] ${
        location.pathname === '/faqs' ? '' : ''
      }`}
    >
      {/* Show this only on /faqs */}
      {location.pathname === '/faqs' && (
        <section className="text-center border w-full mb-3 py-10 bg-white">
      ...
      <section className="py-12 grid grid-cols-12 gap-8 bg-white md:px-[10rem]">
        {/* FAQ Section */}
        <div className="col-span-12 md:col-span-6 flex flex-col">
          <h1 className="text-xl font-bold text-center mb-10">
            Frequently Asked Questions
          </h1>
          <div className="space-y-4 flex-1 max-w-5xl mx-auto">
            {faqs.map((faq, index) => (
        ...
        {/* Image Section */}
        <div className="col-span-12 md:col-span-6 flex items-center justify-center h-full">
          <div className="w-full h-full flex items-center justify-center">
            <img
              src={img}
              alt="FAQs illustration"
              className="max-w-[16rem] w-full object-contain rounded-[10px]"
            />
          </div>
        </div>
      </section>
